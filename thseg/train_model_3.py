    ...
    with open(os.path.join(param_dict['save_dir_model'], 'log.txt'), 'w') as ff:
        ...
        for epoch in range(start_epoch, param_dict['epoches']):
            ...
            for i, data in tqdm.tqdm(enumerate(trainloader)):  # get data
                ...
                dis_real = netD(torch.cat([images, dis_input_real], 1) )
                dis_real_loss = BCE_loss(dis_real, True, True)

                # Discriminator Fake Loss
                outputs = model(images)
                dis_input_fake = outputs.detach() #detach or not? TODO: check
                dis_fake = netD(torch.cat([images, dis_input_fake],1))
                ...
                gen_fake = netD(torch.cat([images, gen_input_fake],1))
            ...
            if epoch % param_dict['save_iter'] == 0:
                val_miou, val_acc, val_f1, val_loss = eval(valloader, model, criterion, epoch) #TODO: be consistent with criterion
...
def eval(valloader, model, criterion, epoch):
    ...
    return IoU[1], OA, f1ccore[1], val_loss #TODO: Why IoU[1] instead of mIOU? TODO: be consistent with loss here in validation and in training
