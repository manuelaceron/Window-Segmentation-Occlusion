...
def main():
    ...
    """ netD = NLayerDiscriminator(input_nc=1)
    netD.cuda()
    optimizerD = create_optimizer_v2(netD, 'adam', lr=param_dict['base_lr'])
    lr_scheduleD = torch.optim.lr_scheduler.StepLR(optimizerD, step_size=50, gamma=0.8)
    gan_criterion = AdversarialLoss('nsgan').cuda() """
    ...
    with open(os.path.join(param_dict['save_dir_model'], 'log.txt'), 'w') as ff:
        ...
        for epoch in range(start_epoch, param_dict['epoches']):
            ...
            #pdb.set_trace()
            print('epoch is {}, train Gen loss is {}, Dis loss is {}, Per epoch time {}'.format(epoch, gen_gan_loss.item() / batch_num, dis_loss.item() / batch_num, per_epoch_ptime))
            cur_lr = optimizer.param_groups[0]['lr']
            writer.add_scalar('learning_rate', cur_lr, epoch)
            writer.add_scalar('train_loss', gen_gan_loss / batch_num, epoch)
            writer.add_scalar('disc_loss', dis_loss / batch_num, epoch)
            #lr_schedule.step()
            if epoch % param_dict['save_iter'] == 0:
                val_miou, val_acc, val_f1, val_loss = eval(valloader, model, nn.BCELoss(), epoch)
                ...
                cur_log = 'epoch:{}, learning_rate:{}, trainG_loss:{}, trainD_loss:{}, val_loss:{}, val_f1:{}, val_acc:{}, val_miou:{}\n'.format(
                    str(epoch), str(cur_lr), str(gen_gan_loss.item() / batch_num), str(dis_loss.item() / batch_num), str(val_loss), str(val_f1),
                    str(val_acc),
                    str(val_miou)
                )
                print(cur_log)
    ...
    with torch.no_grad():
        ...
        for i, data in tqdm.tqdm(enumerate(valloader), ascii=True, desc="validate step"):  # get data
            ...
            if param_dict['extra_loss']:
                ...
                outputs = model(images)
            criterion = get_loss(param_dict['loss_type']) 
