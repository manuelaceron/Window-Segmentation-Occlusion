...
from networks.MAT.torch_utils import misc
from networks.MAT.torch_utils import persistence
from networks.MAT.networks.basic_module import FullyConnectedLayer, Conv2dLayer, MappingNet, MinibatchStdLayer, DisFromRGB, DisBlock, StyleConv, ToRGB, get_style_code
...
class SynthesisNet(nn.Module):
    ...
    def forward(self, images_in, masks_in, ws, noise_mode='random', return_stg1=False):
        ...
        E_features = self.enc(x) # Encoder features for each encoder layer 512 -> 256-> 128 -> 64-> 32 -> 16 (with different depth channel)
        

        fea_16 = E_features[4] #[1, 512, 16, 16])
        ...
        if not return_stg1: # Output of convolutional tail
            return img
        else:
            return img, out_stg1
