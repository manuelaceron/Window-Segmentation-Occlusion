...
def main():
    ...
    criterion_w = get_loss(param_dict['loss_type_w'])#, torch.tensor([0.63147511, 2.40150057]))  # define loss
    criterion_occ = get_loss(param_dict['loss_type_occ'], torch.tensor([0.57487292, 3.83899089]))  # define loss 

    L1_loss = nn.L1Loss().cuda()
    writer = SummaryWriter(os.path.join(param_dict['save_dir_model'], 'runs'))

    best_val_acc = 0.0
    with open(os.path.join(param_dict['save_dir_model'], 'log.txt'), 'w') as ff, open(os.path.join(param_dict['save_dir_model'], 'occ_log.txt'), 'w') as cc:
        ...
        for epoch in range(start_epoch, param_dict['epoches']):
            ...
            for i, data in tqdm.tqdm(enumerate(trainloader)):  # get data
                ...
                images, labels, occ_mask  = data['image'], data['gt'], data['occ'] #First batch of N images 3x5125x512
                ...
                losses_occ = criterion_occ(outputs1, occ_mask) #Occlusion mask labels
                losses_w = criterion_w(outputs2, labels) #Window segmentation labels


                losses = losses_w  + (losses_occ) 
            ...
            print('Loss occlusion is {}, Loss segmentation is {}'.format(losses_occ.item() / batch_num, losses_w.item() / batch_num ))
            #print('Loss segmentation is {}'.format(losses_w.item() / batch_num ))
            ...
            if epoch % param_dict['save_iter'] == 0:
                ...
                results = eval(valloader, model, criterion_w, criterion_occ, epoch)
                ...
                cur_log = 'epoch:{}, learning_rate:{}, train_loss:{}, val_loss:{}, val_f1:{}, val_acc:{}, val_miou:{}\n'.format(
                    str(epoch), str(cur_lr), str(losses_w.item() / batch_num), str(val_loss), str(val_f1),
                    str(val_acc),
                    str(val_miou)
                )

                cc_cur_log = 'epoch:{}, learning_rate:{}, train_loss:{}, val_loss:{}, val_f1:{}, val_acc:{}, val_miou:{}\n'.format(
                    str(epoch), str(cur_lr), str(losses_occ.item() / batch_num), str(val_loss_mask), str(val_f1_mask),
                    str(val_acc_mask),
                    str(val_miou_mask)
                )
                ...
                if epoch >= 50:#val_miou > best_val_acc:
                    checkpoint = {
...
def eval(valloader, model, criterion_w, criterion_occ, epoch):
    ...
    with torch.no_grad():
        ...
        for i, data in tqdm.tqdm(enumerate(valloader), ascii=True, desc="validate step"):  # get data
            ...
            images, labels, img_path, gt_path, gt_path_mask, occ_mask = data['image'], data['gt'], data['img_path'], data['gt_path'], data['occ_path'], data['occ']          
            ...
            vallosses_mask = criterion_occ(out_occ, occ_mask)
